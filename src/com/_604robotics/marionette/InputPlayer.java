...
public class InputPlayer {
  ...
  public void startPlayback(final InputRecording recording) {
    ...
    System.out.println(
        "MARIONETTE PLAYBACK START [frameCount = " + playbackRecording.getFrameCount() + "]");
  }
  ...
  private boolean advanceFrame() {
    ...
    while (playbackFrame < playbackRecording.getFrameCount()
        && elapsedTime >= playbackRecording.getTimestamp(playbackFrame + 1)) {
      System.out.println(
          "* advancing frame ["
              + playbackRecording.getTimestamp(playbackFrame)
              + " -> "
