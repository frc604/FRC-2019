...
public class InputRecorder implements Closeable {
  ...
  public InputRecorder(
      final int maxFrameCount, final long framePause, final GenericHID... sources) {
    this.sources = sources;

    final JoystickDescriptor[] joystickDescriptors = new JoystickDescriptor[sources.length];
    for (int i = 0; i < sources.length; ++i) {
      joystickDescriptors[i] =
          new JoystickDescriptor(sources[i].getAxisCount(), sources[i].getButtonCount());
    }
