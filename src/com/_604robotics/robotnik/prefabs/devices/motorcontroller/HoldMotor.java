...
public class HoldMotor {
  ...
  public HoldMotor(
      SpeedController motor, Encoder encoder, double target_speed, int click_tolerance) {
    this.motor = motor;
    this.encoder = encoder;
    offset = 0;
    increment = 0.0001;
  ...
  public boolean canHold(int click_target) {
    if (click_target + click_tolerance > encoder.get()
        && encoder.get() > click_target - click_tolerance) {
      at_position = true;
    } else {
      at_position = false;
    }
