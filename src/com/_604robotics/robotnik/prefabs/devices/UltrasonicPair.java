...
public class UltrasonicPair implements Ultrasonic {
  ...
  /**
   * Gets the angle of the sensors relative to a wall using 64 samples. To the left is negative,
   * straight-on is zero, and to the right is positive.
   *
  ...
  /**
   * Gets the angle of the sensors relative to a wall. To the left is negative, straight-on is zero,
   * and to the right is positive.
   *
   * @param samples Number of samples to take.
   * @return The angle of the sensors relative to a wall.
   */
  public double getAngle(int samples) {
    double angle =
        Math.acos(
            this.separation
                / Math.pow(
                    (Math.pow(this.separation, 2) + Math.pow(getDifference(samples), 2)), 0.5));
