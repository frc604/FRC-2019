...
public class Pixy extends Module {
  ...
  public PixyPacket readPacket(int Signature)
      throws PixyException { // The signature should be which number object in
    int Checksum; // pixymon you are trying to get data for
    int Sig;
    byte[] rawData = new byte[32];

    SmartDashboard.putString(
        "rawData", rawData[0] + " " + rawData[1] + " " + rawData[15] + " " + rawData[31]);
    ...
    for (int i = 0; i <= 16; i++) {
      ...
      if (syncWord
          == 0xaa55) { // Check is first 2 bytes equal a "sync word", which indicates the start of a
        ...
        // Checks whether the data is valid using the checksum *This if block should never be
        // entered*
        if (Checksum
            != Sig
                + packets[Sig - 1].X
